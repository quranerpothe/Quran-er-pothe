...
module.exports.onStart = async  ({ event, api, args })=> {
    ...
    if (!mention) return api.sendMessage("Please tag 1 person", threadID, messageID);
        ...
        let bg_img = await jimp.read("https://i.imgur.com/SC8LEeb.jpeg");
    ...
    bg_img.resize(1030, 587).composite(circleOne.resize(300, 300).circle(), 122, 145).composite(circleTwo.resize(300, 300).circle(), 600, 145);
         let filePath = `./gf_${one}.png`;
		await bg_img.writeAsync(filePath);
       api.sendMessage({ body: "╔⏤⏤⏤╝❮❮𝐌𝐈𝐌-𝐁𝐎𝐓-𝟎𝟎𝟕❯❯╚⏤⏤⏤╗\n■□■□■□■□■□■\n\nআমরা খুব কাছাকাছি গিয়ে বুঝতে পারি,মানুষ আসলে দূর থেকেই সুন্দর।🤍\n\n■□■□■□■□■□■\n╔⏤⏤⏤╝❮❮𝐌𝐈𝐌-𝐁𝐎𝐓-𝟎𝟎𝟕❯❯╚⏤⏤⏤╗",attachment: fs.createReadStream(filePath) }, threadID, () => fs.unlinkSync(filePath), messageID)
  }
